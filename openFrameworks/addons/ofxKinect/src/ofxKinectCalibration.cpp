...
void ofxKinectCalibration::setClippingRaw(float near, float far){
	for(int i = 0; i < 2048; i++){
		if(i == 2047) {
			...
			depthPixelsLookupNearWhite[i] = ofMap(i, near, far, 255, 0, true);
			depthPixelsLookupFarWhite[i] = 255 - depthPixelsLookupFarWhite[i];
		}
	}
}
