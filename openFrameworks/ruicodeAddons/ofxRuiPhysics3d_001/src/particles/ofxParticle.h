...
public:
	...
	void applyForce(const ofPoint& _force);
	void applyImpulse(const ofPoint& _impulse);
	...
	void moveTo(const ofPoint& _target);
	void moveTo(float _x, float _y, float _z);
	void moveBy(const ofPoint& _amount);
	void moveBy(float _x, float _y, float _z);
	void lerp(const ofPoint& _target, float _interp);
	void moveTowards(const ofPoint& _target, float strength);
	void applyAttractionForce(const ofPoint& target, float amount);
	void applyRepulsionForce(const ofPoint& target, float amount);
	void stopMotion();
	
	void setVel(const ofPoint& _vel);
	ofPoint getVel();
