...
//a spring that only activates once the distance between the particles is smaller than a determined value

class ofxMinDistSpring:public ofxConstraint{
protected:

public:
	ofxMinDistSpring(ofxParticle *_a, ofxParticle* _b, float _rest, float _strength = 1.0f):
	ofxConstraint(_a, _b, _rest, _strength, OFX_MIN_DIST_SPRING){
	};
	...
	virtual void update(){
		...
		if(aRef.isActive() && bRef.isActive()){
			...
			moveAmount = ((dist - rest) / dist)*0.5f;
			if(aRef.isActive()){
