...
ofxShader AtlasGSDrawer::shader;
bool AtlasGSDrawer::shaderInited = false;

AtlasGSDrawer::AtlasGSDrawer(){
	allocated = false;	
	quadCount = 0;
	vertexBuffer = NULL;
	texCoordBuffer = NULL;
	...
	if (!shaderInited) {
		//shader.loadShaderGS("shaders/AtlasGSDrawer.fs", "shaders/AtlasGSDrawer.vs", "shaders/AtlasGSDrawer.gs");
		loadShaders(); 
		shaderInited = true;
	}
...
void AtlasGSDrawer::loadShaders(){
	shader.loadShaderGS("shaders/AtlasGSDrawer.fs", "shaders/AtlasGSDrawer.vs", "shaders/AtlasGSDrawer.gs");
}

void AtlasGSDrawer::setTextureAtlas(TextureAtlas& textureAtlas){
...
void AtlasGSDrawer::setup(int quadCount){
...
void AtlasGSDrawer::setupGUI(){
	blendSetting.setupGUI();
	gui.addToggle("debug draw",doDebugDraw);
}

void AtlasGSDrawer::draw(vector<Particle*>& particles){
...
void AtlasGSDrawer::draw(int count){
