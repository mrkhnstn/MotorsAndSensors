...
void ofxSimpleGuiComboBox::loadFromXML(ofxXmlSettings &XML) {
	if(includeInXml)
		setValue(XML.getValue(controlType + "_" + key + ":value", m_defaultChoice));
}
...
void ofxSimpleGuiComboBox::incrementChoice(){
	int temp = m_selectedChoice+1;
	if (temp >= m_choices.size()) {
		temp = 0;
	}
	m_selectedChoice = temp;
};

void ofxSimpleGuiComboBox::decrementChoice(){
	int temp = m_selectedChoice-1;
	if (temp < 0) {
		temp = m_choices.size()-1;
	}
	m_selectedChoice = temp;	
};

void ofxSimpleGuiComboBox::onKeyLeft(){ decrementChoice(); };
void ofxSimpleGuiComboBox::onKeyRight(){ incrementChoice(); };
void ofxSimpleGuiComboBox::onKeyUp(){ 
	//decrementChoice(); 
};
void ofxSimpleGuiComboBox::onKeyDown(){ 
	//incrementChoice();
};
