...
void ofxFBOTexture::allocate(int w, int h, int internalGlDataType, int numSamples, bool useARB) {
	_isActive = false;
    ...
    if (GLEE_ARB_texture_rectangle && useARB){
        texData.tex_w = w;
        texData.tex_h = h;
...
void *ofxFBOTexture::getPixels() {
	...
	glReadPixels(0, 0, texData.width, texData.height, texData.glType, texData.glTypeInternal, pixels);
