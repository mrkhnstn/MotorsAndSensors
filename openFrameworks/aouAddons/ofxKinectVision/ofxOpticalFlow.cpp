/*
------------------------------------------------------------------------------------------------
 ofxOpticalFlow.cpp | ofxKinect | 07/02/2011
...
void ofxOpticalFlow::setup(int _imgWidth, int _imgHeight, int _windowWidth, int _windowHeigth){
	clear();
	width = _imgWidth;
	height = _imgHeight;
...
void ofxOpticalFlow::draw(float x, float y){
	vector<float> coords;
	...
	for(int i=0; i<coords.size(); i+=4){
		ofCircle(coords[i], coords[i+1], 2);
	}
	
}

void ofxOpticalFlow::clear(){
...
int ofxOpticalFlow::getNumFlowPoints(){
	return flowPoints.size();	
}

vector<ofxCvFlowPoint>& ofxOpticalFlow::getFlowPoints(){
	return flowPoints;
}

void ofxOpticalFlow::normalizePoints(){
	for(int i=0; i<flowPoints.size(); i++){
...
void ofxOpticalFlow::scalePoints(float scaleX, float scaleY){
	for(int i=0; i<flowPoints.size(); i++){
...
void ofxOpticalFlow::filterPoints(float minFlow, float maxFlow){
...
ofPoint ofxOpticalFlow::getAverageFlow(){
	ofPoint avg;
	for(int i=0; i<flowPoints.size(); i++){
