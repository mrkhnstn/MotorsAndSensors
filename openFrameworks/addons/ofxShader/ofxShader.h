...
public:
	...
	/** if attributeName is omitted or "" then no attribute will be bound (note this allows only to bind one attribute to the vertex shader*/
	void 	loadShader(string fragmentShaderName, string vertexShaderName, string attributeName = ""); 
	
	// load with geometry shader
	void	loadShaderGS(string fragmentShaderName, string vertexShaderName, string geometryShaderName);
